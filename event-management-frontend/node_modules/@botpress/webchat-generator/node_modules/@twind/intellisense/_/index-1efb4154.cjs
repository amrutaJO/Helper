...
const matchSorter = require('match-sorter'), cssbeautify = require('cssbeautify'), QuickLRU = require('quick-lru'), core = require('@twind/core'), genex = require('genex'), tinycolor = require('@ctrl/tinycolor'), _interopDefault = (e)=>e && e.__esModule ? e : {
        default: e
    }, cssbeautify__default = /*#__PURE__*/ _interopDefault(cssbeautify), QuickLRU__default = /*#__PURE__*/ _interopDefault(QuickLRU), genex__default = /*#__PURE__*/ _interopDefault(genex);
/*
To set a bit: n |= mask;
...
const simplePseudoClasses = [
    ...
    ':valid',
    ':visited'
], VARIANT_MARKER_RULE = /^…$/, colorNames = Object.keys(tinycolor.names), editabelColorRe = RegExp(`-\\[(${colorNames.join('|')}|(?:(?:#|(?:(?:hsl|rgb)a?|hwb|lab|lch|color)\\())[^]\\(]+)\\]$`, 'i'), colorCache = new QuickLRU__default.default({
    maxSize: 1000
});
function parseColor(value) {
    let result = colorCache.get(value);
    if (!result) {
        let color = new tinycolor.TinyColor(value);
...
function compareSuggestions(a, b, prefix) {
    ...
    // by min width
    if (isVariant) {
        let aMinWidth = a.description?.startsWith('@') && a.description.includes('min-width'), bMinWidth = b.description?.startsWith('@') && b.description.includes('min-width');
...
exports.createIntellisense = function(config, options = {}) {
    let suggestionCache = new QuickLRU__default.default({
        maxSize: 1000,
        ...options.cache
    }), documentationCache = new QuickLRU__default.default({
        maxSize: 1000,
        ...options.cache
    }), context = function(config, options = {}) {
        let tw = core.twind(core.defineConfig({
            hash: false,
            preflight: false,
            // disable autoprefix
            ...
            presets: [
                core.defineConfig('function' == typeof config ? config.config : config)
            ],
            rules: [
        ...
        }), core.virtual(true)), ignorelist = core.asArray(tw.config.ignorelist).map(toCondition), isIgnored = (className)=>ignorelist.some((re)=>re.test(className)), variants = new Map(), classes = new Map(), suggestions = [], cssCache = new QuickLRU__default.default({
            maxSize: 1000,
            ...options.cache
        ...
        for (let screen of Object.keys(tw.theme('screens'))){
            ...
            add(variants, {
                ...
                description: core.mql(tw.theme('screens', screen))
            });
        }
        for (let [pattern, resolver] of tw.config.variants){
            let index = nextIndex++, position = 0, provider = 'function' == typeof resolver && core.getAutocompleteProvider(resolver);
            for (let value of core.asArray(pattern)){
                let condition = toCondition(value), source = condition.toString(), re = RegExp(condition.source.replace(/\\[dw][*+?]*/g, '\0'), condition.flags), pattern1 = genex__default.default(re), count = pattern1.count();
                count === 1 / 0 ? provider ? deferreds.push(()=>{
                    ...
                    if (match) {
                        ...
                        if (provider) for (let completion of provider(match, context))'string' == typeof completion ? add(variants, {
                            type: 'variant',
                            source,
                            index,
                            position: position++,
        ...
        for (let rule of tw.config.rules){
            let [pattern2, resolver1] = core.asArray(rule), index1 = nextIndex++, position1 = 0, provider1 = 'function' == typeof resolver1 && core.getAutocompleteProvider(resolver1);
            for (let value1 of core.asArray(pattern2)){
                if (value1 === VARIANT_MARKER_RULE) continue;
                let condition1 = toCondition(value1), source1 = condition1.toString(), re1 = RegExp(condition1.source.replace(/\\[dw][*+?]*/g, '\0'), condition1.flags), pattern3 = genex__default.default(re1), count1 = pattern3.count();
                if (count1 === 1 / 0) {
                    if (provider1) {
                        let match = Object.create([
                            String(value1)
                    ...
                    if (match) {
                        ...
                        if (provider1) for (let completion of provider1(match, context))'string' == typeof completion ? add(classes, {
                            type: 'class',
                            source: source1,
                            index: index1,
                            position: position1++,
        ...
        return suggestions.sort(compareSuggestions), {
            ...
            generateCSS: (token)=>{
                ...
                if (!result) {
                    ...
                    tw(name);
                    let css = core.stringify(tw.target), needle = `,${name}*/`, startIndex = css.indexOf(needle);
    ...
    function toSuggestion(suggestion) {
        return 'variant' === suggestion.type ? {
            ...
            description: (_suggestion = suggestion).description || (_suggestion.description = function(className) {
                ...
                return -1 === bodyStart || -1 === bodyEnd ? css : cssbeautify__default.default(`.x{${css.slice(bodyStart + 1, bodyEnd)}}`, {
                    indent: ''
                }).split('\n').slice(1, -1).join(' ');
            }(suggestion.name)),
            detail: suggestion.detail,
    ...
    let mdnIndexPromise = null, mdnCache = new QuickLRU__default.default({
        maxSize: 1000,
        ...options.cache
    });
    return {
        ...
        async suggest (input, { prefix ='' , ignore  } = {}) {
            ...
            if (!result) {
                let source = context.suggestions, threshold = matchSorter.matchSorter.rankings.MATCHES, token = prefix + input, match = /^(.+(?:-\[.+])?)\/([^/]+|\[.+])?$/.exec(token);
                if (match) {
                ...
                ignore?.length && (source = source.filter(({ value  })=>!ignore.includes(value)));
                ...
                if (0 === length) {
                    ...
                    length < 2 && (threshold = matchSorter.matchSorter.rankings.CONTAINS);
                    // TODO: spacify should ignore `[...]`
                    let search = spacify(input);
                    suggestionCache.set(key, result = matchSorter.matchSorter(source, search, {
                        keys: [
                            'filter'
                        ],
                        threshold,
        ...
        async suggestAt (content, position, language) {
            ...
            let { extractBoundary  } = 'html' === language ? await Promise.resolve().then(()=>require('./html-549675df.cjs')) : {
            ...
            let isEmptyPosition = !boundary.content || /[\s():/!-@]$/.test(boundary.content), parsed = core.parse(boundary.content + (isEmptyPosition ? '\uffff' : '')), rule = parsed[parsed.length - 1];
        ...
        async documentationFor (token, { format ='md'  } = {}) {
            ...
            let rule = core.parse(token)[0] || {
                n: token.endsWith(':') ? '' : token,
                v: token.endsWith(':') ? [
                    token
                ] : []
            }, css = cssbeautify__default.default(context.generateCSS(token), {
                autosemicolon: true,
                indent: '  '
            ...
            mdnIndex = await (mdnIndexPromise || (mdnIndexPromise = (async ()=>{
                ...
                let { title , browserCompat  } = await cached, links = [
                    ...
                    browserCompat?.[0] && `[Browser Support](https://caniuse.com/mdn-${browserCompat[0].replace(/\./g, '_')})`
                ].filter(Boolean).join(' • ');
                return `⁃ \`${title}\` (${links})`;
            })), sources = [];
            ...
            for (let completion of [
                ...
                if (completion.theme) {
                    ...
                    obj = obj[keys.shift()] = keys.length ? {} : context.tw.theme(section, key) ?? '…';
                }
            }
            let result = [
                css && '```css\n' + css + '\n```',
        ...
        async documentationAt (content, offset, language) {
            let result = null;
            if ('html' === language) {
                let { documentationAt  } = await Promise.resolve().then(()=>require('./html-549675df.cjs'));
        ...
        async collectColors (content, language) {
            if ('html' === language) {
                let { collectColors  } = await Promise.resolve().then(()=>require('./html-549675df.cjs'));
        ...
        async validate (content, language) {
            if ('html' === language) {
                let { validate  } = await Promise.resolve().then(()=>require('./html-549675df.cjs'));
...
exports.editabelColorRe = editabelColorRe;
exports.parseColor = parseColor;
//# sourceMappingURL=index-1efb4154.cjs.map
